  ...
  \begin{columns}[T]
    ...
    \begin{block}{Conexidade em grafos dinâmicos}
     O problema da conexidade em grafos dinâmicos visa uma implementação eficiente da biblioteca abaixo:
      ...
      \begin{shaded}
        \textbf{Ideia:} Fatiar o grafo $G$ em níveis. Cada aresta de $G$ possui um nível entre 1 e $\left\lceil \lg n \right\rceil$, onde $n$
        é o número de vértices de $G$. Uma aresta, ao ser inserida em $G$, começa com o nível $\left\lceil \lg n \right\rceil$ e, durante o algoritmo, seu nível vai sendo decrementado. Seja $G_i$ o subgrafo de $G$ com as arestas de $G$ de nível menor ou igual a $i$. Para cada nível $i$, o algoritmo mantém uma floresta maximal $F_i$ de $G_i$. Além disso, ele mantém também o subgrafo $R_i$ de $G$ com as arestas de nível $i$ que não estão em $F_i$, chamadas de arestas reserva.
      \end{shaded}
      ...
      \begin{figure}[H]
          ...
          \begin{minipage}{0.7\textwidth} % Usa aproximadamente 70% da largura do texto
              ...
              % Adiciona uma label ou descrição para o primeiro gráfico
                    \node[font=\small] at (-1, 1) {\textcolor{blue}{$T$}};
              ...
              \draw[->, blue, very thick] ([yshift=0cm, xshift=1cm]d1.east) 
                  to[out=-20, in=20] ([yshift=-3.2cm]d1.south); % Start from 'b.east', 
                \node[blue, font=\small, fill=white, inner sep=2pt] at (10.5, -3.7) {trilha Euleriana};
          ...
          \begin{minipage}{\textwidth} 
              ...
              \begin{tikzpicture}
                    ...
                    \node[font=\small] at (-4, 0.9) {\textcolor{blue}{Euler tour}};
                    \node[font=\small] at (-4, 0) {\textcolor{blue}{tree de $T$}};
              \end{tikzpicture}
...
\begin{itemize}
    ...
    \item[$\bullet$] \label{invariant3} Cada componente de $F_i$ tem no máximo $2^i$ vértices.
\end{itemize}
\end{shaded}
    \end{block}
...
\end{figure}
    ...
    \begin{block}{MSF decremental}
      ...
      \definecolor{shadecolor}{rgb}{0.74, 0.83, 0.9} % pale blue
      \begin{shaded}
        \textbf{Ideia:} No loop das linhas $10$ a $15$ do método \texttt{substituaAresta}, busca-se por uma aresta substituta numa ordem arbitrária. Na implementação da MSF decremental, a ideia é olhar as arestas candidatas à substituta em ordem crescente de peso. Para isso, as listas de adjacências de $R_i$ são substituídas por min-heaps, onde a chave para um vizinho $v$ é o peso da aresta $uv$. Assim, nas linhas $11$ e $14$ do \texttt{substituaArestaMSF}, basta procurar por uma aresta em $R_i$ que incida em $T_u$, que tenha o menor peso e que conecte $T_u$ e $T_v$, para podermos manter o peso mínimo de uma MSF do grafo ao longo das remoções de arestas. 
      \end{shaded}
    \end{block}
    ...
    \begin{block}{}
      ...
      Na figura abaixo, veja que, nas linhas $11$ e $14$, passamos a procurar uma aresta de peso mínimo em $R_i$ incidente a $T_u$. As linhas $1$ a $10$ são idênticas às do método \texttt{substituaAresta}.
    ...
    \definecolor{shadecolor}{rgb}{0.93, 0.80, 0.82} % pale pink
      \begin{shaded}
        \begin{itemize}
            \item[$\bullet$] \texttt{\textbf{MSFDecremental($n$, $E$)}}: $\Oh(|E| \lg n)$;
            \item[$\bullet$] \texttt{\textbf{consultePesoMSF($G$)}}: $\Oh(1)$;
            \item[$\bullet$]  \texttt{\textbf{removaMSF($G$, $u$, $v$)}}: amortizado $\Oh(\lg^2 n)$.
        \end{itemize} 
      \end{shaded}
      ...
      \begin{figure}[htb]
          ...
          \includegraphics[width=1\linewidth]{./figuras/graph_0_permutation_0.png}
          
          \label{prog:chart_performance} 
      \end{figure}

      \begin{figure}[htb]
          \centering
  
          \includegraphics[width=1\linewidth]{./figuras/performance_table_graph_0_permutation_0.png}
          
          \label{prog:graph_performance} 
      \end{figure}

    \end{block}

    \begin{block}{Comentários finais}
    No texto do TCC, além da descrição detalhada dos algoritmos apresentados aqui bem como de suas análises de consumo de tempo, também descrevemos brevemente a ideia por trás de um algoritmo para o problema da MSF totalmente dinâmico, que dá suporte  eficiente não só à remoção de arestas, como também à inserção de arestas. Esse algoritmo faz uso do algoritmo da MSF decremental apresentado acima.
    \end{block}
