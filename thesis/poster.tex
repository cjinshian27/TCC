...
Com isso, podemos estabelecer algumas invariantes, que são mantidas ao longo das modificações no grafo $G$:
      ...
      \begin{shaded}
\begin{itemize}
    \item[$\bullet$] \label{invariant1} $F_i$ é uma floresta maximal de $G_i$ para todo $1 \leq i \leq  \left\lceil \lg n \right\rceil$;
    
    \item[$\bullet$] \label{invariant2} $F_i \subseteq F_{i+1}$ para todo $1 \leq i \leq \left\lceil \lg n \right\rceil - 1$;
    
    \item[$\bullet$] \label{invariant3} Cada componente da floresta $F_i$ possui no máximo $2^i$ vértices.
\end{itemize}
\end{shaded}
    \end{block}
    ...
    Abaixo temos uma simplificação do código da remoção de uma aresta do grafo. Observe que as Euler tour trees carregam informação extra para que a implementação das linhas $7$, $8$, $12$ e $13$ do método \texttt{substituaAresta} seja eficiente. 
    ...
    \caption{Algoritmo de remoção de aresta do grafo.}
...
\end{figure}
    ...
    \caption{Algoritmo que procura uma aresta substituta.}
...
\begin{figure}[htb]
    ...
    \caption{Algoritmo que testa se uma aresta reserva é substituta.}
    \label{prog:testSubstitute_image} % Use a meaningful label for referencing
\end{figure}
      
    Como a nossa implementação utiliza splay trees, algumas rotinas da biblioteca possuem um consumo amortizado por operação.
    ...
    \begin{block}{MSF decremental}
     ...
     O problema da manutenção de uma MSF decremental do grafo visa uma implementação eficiente dos métodos da biblioteca abaixo:
      ...
      \definecolor{shadecolor}{rgb}{0.74, 0.83, 0.9} % pale blue
      \begin{shaded}
        \textbf{Ideia:} remover uma aresta de nível $i$ da floresta $F_i$ , quebra uma componente desta em duas, $T_u$ e $T_v$.  Para isso, precisamos buscar por uma aresta substituta que tenha o menor peso e que ligue $T_u$ a $T_v$. Assim, em vez de usar um mapa hash (como fazemos no algoritmo de conexidade do grafo dinâmico) para armazenar os vizinhos de cada vértice, usaremos um min-heap, onde a chave dessa estrutura de dados para um vizinho $v$ será o peso da aresta $uv$. Dessa forma, basta procurar por uma aresta em $R_i$ que incide em $T_u$ e que tenha o menor peso que conecte $T_u$ e $T_v$.
      \end{shaded}

      Veja que, na Figura~\ref{prog:replaceGDMSF_image}, nas linhas $13$ e $17$ passamos a procurar a aresta de peso mínimo em $R_i$ incidente a $T_u$.
      ...
      \begin{figure}[htb]
          ...
          \caption{Algoritmo da MSF decremental que procura uma aresta substituta.}
    ...
    \definecolor{shadecolor}{rgb}{0.93, 0.80, 0.82} % pale pink
      ...
      A seguir, testamos a remoção das arestas de um grafo $G$ com 300.000 vértices e 300.000 arestas, realizando uma comparação da performance do algoritmo de Kruskal e da MSF decremental que implementamos. 
