...
\end{figure}
    ...
    \begin{block}{MSF decremental}
      ...
      \definecolor{shadecolor}{rgb}{0.74, 0.83, 0.9} % pale blue
      \begin{shaded}
        \textbf{Ideia:} remover uma aresta de nível $i$ da floresta $F_i$, quebra uma componente desta em duas, $T_u$ e $T_v$. Precisamos buscar uma aresta substituta \textbf{com o menor peso} que ligue $T_u$ a $T_v$. Em vez de usar listas de adjacências para armazenar os vizinhos de cada vértice em $R_i$, usa-se um min-heap, onde a chave para um vizinho $v$ é o peso da aresta $uv$. Assim, basta procurar por uma aresta em $R_i$ que incida em $T_u$, que tenha o menor peso e que conecte $T_u$ e $T_v$, para podermos manter o peso mínimo de uma MSF do grafo ao longo das remoções de arestas. 
      \end{shaded}
    \end{block}
    ...
    \begin{block}{}
      ...
      Na figura abaixo, veja que, nas linhas $11$ e $14$, passamos a procurar a aresta de peso mínimo em $R_i$ incidente a $T_u$. Note também que as linhas $1$ ao $9$ são idênticas às do método \texttt{substituaAresta}.
