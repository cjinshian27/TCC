...
Como usaremos o método \texttt{exponhaTopTree} no algoritmo da MSF totalmente dinâmica, vale mostrar um exemplo de como ficaria a top tree da Figura~\ref{fig:top-t-from-tree-t} ao chamarmos \texttt{exponhaTopTree($7$, $8$)}, ou seja, os vértices $7$ e $8$ passam a ser os novos vértices da fronteira de $T$.
...
\subsection{MSF totalmente dinâmica}

A Seção 5.1 do artigo de Holm, de Lichtenberg e Thorup~\cite{jacob_holm} propõe um algoritmo para a MSF totalmente dinâmica, a qual mantém uma floresta geradora maximal de custo mínimo que consome tempo amortizado $\Oh(\lg^4 n)$ por inserção e remoção de aresta.
