...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Representação de uma árvore $T$ de $5$ vértices. Os vértices pintados em vermelho são vértices da fronteira de $T$. A região demarcada em vermelho representa a subárvore $S$ contendo os vértices $a$, $e$ e $d$ e as arestas $ae$ e $ad$. Neste exemplo, note que os vértices da fronteira de $S$ são $a$ e $d$.}
...
Dizemos que clusters $A$ e $B$ são vizinhos se eles compartilham um único vértice entre eles e se $A \cup B$ é um cluster, como podemos ver na Figura~\ref{fig:clusters-formation-cases}
...
% (3)
\node[boundary] (a3) at (0,-2.2) {};
\node[internal] (b3) at (1.2,-2.2) {};
\node[boundary] (c3) at (2.4,-2.2) {};

\draw[edge, bend left=30] (a3) to (b3);
\draw[edge, bend right=30] (a3) to (b3);
\draw[edge, bend left=35] (b3) to (c3);
\draw[edge, bend right=35] (b3) to (c3);
\draw[dashededge] (a3) -- (c3);
\node at (1.2, -3) {3};

% (4)
\node[boundary] (a4) at (4,-2.2) {};
\node[internal] (b4) at (4.6,-1.6) {};
\node[internal] (c4) at (5.2,-2.2) {};

\draw[edge, bend left=25] (a4) to (b4);
\draw[edge, bend right=25] (a4) to (c4);
\draw[edge, bend left=25] (b4) to (c4);
\node at (4.6, -3) {4};

\end{tikzpicture}
\caption{Representação da união de dois clusters $A$ e $B$ formando um cluster $C$, e seus quatro diferentes casos. Os vértices pretos são vértices da fronteira de $C$ após a união de $A$ e $B$. Já os vértices brancos são vértices da fronteira de $A$ e de $B$, mas que não tornaram vértices da fronteira de $C$ após a união.}
