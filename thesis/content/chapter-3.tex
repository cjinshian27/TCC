...
\begin{itemize}
    \item \texttt{\textbf{adicioneTopTree($u$, $v$)}}: liga o vértice $u$ ao vértice $v$ se ambos estão em top trees diferentes, criando a aresta ($u$, $v$);
    
    \item \texttt{\textbf{removaTopTree($e$)}}: remove a aresta $e$ da floresta dinâmica;
    
    \item \texttt{\textbf{exponhaTopTree($u$, $v$)}}: retorna \texttt{NIL} se $u$ e $v$ não estão na mesma árvore. Caso contrário, $u$ e $v$ se tornarão vértices da fronteira da top tree que os contém e retorna uma nova cluster raiz (\textit{root cluster}).
\end{itemize}
