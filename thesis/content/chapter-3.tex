...
Assim, dado ($T$, $\partial T$), qualquer subárvore $C$ de $T$ tem um conjunto $\partial_{(T, \partial T)}S$ de vértices da fronteira, que são vértices de $C$ que estão ou em $\partial T$ ou são incidentes a uma aresta de $T$ saindo de $C$, como se pode ver no exemplo da Figura~\ref{fig:top-tree-subtree-option1}.
...
Além disso, se $R$ é uma subárvore de $C$, isto é, $\partial_{(C, \partial_{(T, \partial T)}C)}R = \partial_{(T, \partial T)} R$, então $R$ é um cluster  
 de ($C$, $\partial_{(T, \partial T)} S$) se, e somente se, $R$ é um cluster de $(T, \partial T)$. Podemos ver essa situação no exemplo da Figura~\ref{fig:top-tree-cluster-of-cluster}.
...
\begin{itemize}
    ...
    \item Se $C$ é pai de $A$ e de $B$ em $\mathcal{T}$, então $S = A \cup B$ e $A$ e $B$ são vizinhos;
    \item a raiz de $\mathcal{T}$ é a própria árvore $T$.
\end{itemize}

Para um cluster $C$, os vértices em $S\backslash \partial S$ são chamados de vértices internos (\textit{internal vertices}). Se $a$ e $b$ são vértices da fronteira de $C$, o caminho $a \cdots b$ se chama caminho do cluster (\textit{cluster path}) e denotamos por $\pi(S)$. Se $a \neq b$, o cluster é chamado de \textit{path cluster}. O cluster $C$ é dito caminho ancestral (\textit{path ancestor}) do cluster $$
