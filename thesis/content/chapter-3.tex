...
Seja ($T$, $\partial T$) um par onde $T$ é uma árvore e $\partial T$ é um conjunto que contém no máximo dois vértices de $T$, que são chamados \textbf{vértices da fronteira} (\textit{external boundary vertices}), como se pode ver na Figura~\ref{fig:top-tree-pair-representation}.
...
Para qualquer subárvore $C$ de $T$, seja $\partial_{(T, \partial T)}C$ o conjunto dos vértices de $C$ que estão ou em $\partial T$ ou são incidentes a uma aresta de $T$ saindo de $C$. Chamamos o conjunto $\partial_{T, \partial T}C$ de vértices da fronteira de $C$. Veja o exemplo da Figura~\ref{fig:top-tree-subtree-option1}.
...
Uma subárvore $C$ é chamada de um \textbf{cluster} de ($T$, $\partial T$) se o número de vértices da fronteira de $C$ é no máximo dois. Assim, por definição, $T$ é um cluster dele mesmo, com $\partial_{(T, \partial T)}T = \partial T$. Além disso, qualquer aresta de $T$ é um cluster de ($T$, $\partial T$). A subárvore $C$ da Figura~\ref{fig:top-tree-subtree-option1} acima é um cluster, pois ela possui apenas dois vértices da fronteira.
...
Além disso, se $R$ é uma subárvore de $C$, então $R$ é um cluster  
 ...
 \begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        % \draw[dashed, thick, red] (e) -- (a) -- (d) -- cycle;
    \end{tikzpicture}
    \caption{A região demarcada em azul representa a subárvore $C$ de $T$, que é um cluster de $(T, \partial T)$. Já a região demarcada em verde representa uma subárvore $R$ de $C$, e é um cluster de $(C, \partial C)$ e de $(T, \partial T)$.}
    \label{fig:top-tree-cluster-of-cluster}
\end{figure}

Dizemos que clusters $A$ e $B$ são \textbf{vizinhos} se eles compartilham um único vértice e se $A \cup B$ é um cluster. Finalmente, definimos uma \textbf{top tree} do par ($T$, $\partial T$) como sendo uma árvore binária $\mathcal{T}$ que respeita o seguinte:
...
Para um cluster $C$, os vértices em $C\backslash \partial C$ são chamados de \textbf{vértices internos}. Se $a$ e $b$ são vértices da fronteira de $C$, o caminho entre $a$ e $b$ em $C$ se chama \textbf{caminho do cluster} e o denotamos por $\pi(C)$. Se $a \neq b$, o cluster é chamado de \textbf{cluster-caminho}. 

Dizemos que o cluster $C$ é \textbf{ancestral de caminho} do cluster $D$ e $D$ é chamado de \textbf{descendente de caminho} de $C$ se ambos forem clusters-caminho e $\pi(D) \subseteq \pi(C)$. Note que cada aresta $e \in \pi(C)$ é um descendente de caminho de $C$. Um filho que é descendente de caminho é um \textbf{filho de caminho}. Na Figura~\ref{fig:clusters-formation-cases}, apresentamos os quatro casos que podem ocorrer quando fazemos a união de dois clusters $A$ e $B$. Em (1), $A$ e $B$ são ambos filhos de caminho; em (2), $A \cup B$ tem um filho de caminho; e em (3) e (4), $A \cup B$ não é cluster caminho, logo não há nenhum filho de caminho.
...
% (1)
\node[draw=none] (A1) at (-4.4,0.5) {A};
\node[draw=none] (B1) at (-3.2,0.5) {B};

\node[boundary] (a1) at (-5,0) {};
\node[internal] (b1) at (-3.8,0) {};
\node[boundary] (c1) at (-2.6,0) {};
...
\node[draw=none] at (-3.8, -0.8) {(1)};

% (2)
\node[draw=none] (A2) at (2, 0.5) {A};
\node[draw=none] (B2) at (2.8, 0.5) {B};

\node[boundary] (a2) at (1.3,0) {};
\node[internal] (b2) at (2.5,0) {};
...
\node[draw=none] at (2, -0.8) {(3)};

% (3)
\node[draw=none] (A3) at (-0.9, 0.5) {A};
\node[draw=none] (B3) at (-0.2, 0.5) {B};

\node[boundary] (a3) at (-1.5, 0) {};
\node[boundary] (b3) at (-0.3, 0) {};
...
\node[draw=none] at (-0.8, -0.8) {(2)};

% (4)
\node[draw=none] (A4) at (4.6, 0.5) {A};
\node[draw=none] (B4) at (5.8,0.5) {B};

\node[boundary] (a4) at (5.2, 0) {};
...
\node[draw=none] at (5.2, -0.8) {(4)};

\end{tikzpicture}
\caption{Representação da união de dois clusters $A$ e $B$ formando um cluster $C$, e seus quatro diferentes casos. Os vértices pretos representam vértices da fronteira de $C$ após a união de $A$ e $B$. Já os vértices brancos são vértices da fronteira de $A$ e de $B$, que não são vértices da fronteira de $C$. A linha tracejada é o caminho formado entre os vértices da fronteira de $C$.}
    \label{fig:clusters-formation-cases}
\end{figure}

Ademais, se $a$ é um vértice de fronteira de $C$ e $C$ possui dois filhos $A$ e $B$, então $A$ é considerado mais próximo de $a$ se $a \notin B$. Caso $\partial C = \partial A = \partial C$, então o cluster mais próximo é escolhido arbitrariamente. Dadas essas definições, podemos ilustrar como ficaria uma top tree para a árvore $T$ da Figura~\ref{fig:tree-t}.
...
A partir da árvore $T$, podemos derivar uma top tree, como se pode ver na Figura~\ref{fig:top-t-from-tree-t}. 
...
Como usaremos o método \texttt{exponhaTopTree} no algoritmo da MSF dinâmica, vale mostrar um exemplo de como ficaria a top tree da Figura~\ref{fig:top-t-from-tree-t} ao chamarmos \texttt{exponhaTopTree($7$, $8$)}, ou seja, os vértices $7$ e $8$ passam a ser os novos vértices da fronteira de $T$.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \node[node] (five) at (0, 0) {$5$};
        \node[node] (four) at (-4, -1) {$4$};
        \node[node, fill=red!30] (seven) at (4, -1) {$7$};
        \node[node] (six) at (0, -2) {$6$};
        \node[node] (three) at (-5, -3.5) {$3$};
        \node[node] (one) at (-6, -6) {$1$};
        \node[node] (two) at (-4, -6) {$2$};
       
        \node[node, fill=red!30] (eight) at (2, -3.5) {$8$};
        \node[node] (nine) at (6, -3.5) {$9$};
        \node[node] (ten) at (6, -6) {$10$};
    ...
    \end{tikzpicture}
    \caption{Os vértices $7$ e $8$ em vermelho correspondem agora aos vértices da fronteira de $T$, após acionarmos \texttt{exponhaTopTree($7$, $8$)}.}
    \label{fig:exposed-tree}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Top tree $\mathcal{T}$ da Figura~\ref{fig:top-t-from-tree-t} após acionarmos \texttt{exponhaTopTree($7$, $8$)}, gerando a remoção e reconstrução de alguns clusters através de \texttt{splits} e \texttt{joins}.}
    \label{fig:exposed-top-tree}
\end{figure}

\subsection{MSF dinâmica}

A Seção 5.1 do artigo de Holm, de Lichtenberg e Thorup~\cite{jacob_holm} propõe um algoritmo para a MSF dinâmica, a qual mantém uma floresta geradora maximal de custo mínimo que consome tempo amortizado $\Oh(\lg^4 n)$ por inserção e remoção de aresta.

A ideia inicial é manter um número logarítmico de estruturas decrementais (onde cada uma é uma MSF decremental, que já vimos na Seção~\ref{sec:decremental-msf-library}). Quando queremos remover uma aresta, tal aresta é removida de todas as estruturas decrementais e uma aresta substituta é obtida dentre as arestas substitutas retornadas por estas estruturas. Já no momento da inserção de uma aresta, unimos tal aresta com algumas estruturas decrementais para criar uma nova estrutura decremental.

Inicialmente, manteremos uma MSF $F$ do grafo $G$ com $m$ arestas e um conjunto $\mathcal{G} = \{G_0, \ldots, G_L\}$ de subgrafos de $G$, onde $L = \left\lceil \lg m \right\rceil$. Para cada $G_i$, manteremos uma MSF $F_i$. Chamaremos as arestas de $F$ de \textbf{arestas da floresta globais} e as arestas de $F_i$ de \textbf{arestas da floresta locais}. Todas as arestas de $G$ estarão em pelo menos um $G_i$, então $F \subseteq \bigcup_i F_i$.   

Além disso, temos o grafo $R$ de $G$, que guardará as \textbf{arestas reserva globais}, e $R_i$ de $G_i$, que guardará \textbf{arestas reserva locais}. Com essas definições, podemos enunciar a seguinte invariante:

\begin{enumerate}[label=(\Roman*)]
    \item \label{invariant111} Para cada aresta reserva global $e \in R$, existe exatamente um $i$ tal que $e \in R_i$ e, se $e \in F_j$, então $j > i$.
\end{enumerate}

Dessa forma, se uma aresta $e$ da floresta global é removida, podemos encontrar uma aresta $f$ substituta de menor peso ao removermos todas as ocorrências de $e$ em $G_i$. 

Podemos descrever os métodos de inserção e remoção 