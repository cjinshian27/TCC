...
Além disso, se $R$ é uma subárvore de $C$, isto é, $\partial_{(C, \partial_{(T, \partial T)}C)}R = \partial_{(T, \partial T)} R$, então $R$ é um cluster  
 de ($C$, $\partial_{(T, \partial T)} C$) se, e somente se, $R$ é um cluster de $(T, \partial T)$. Podemos ver essa situação no exemplo da Figura~\ref{fig:top-tree-cluster-of-cluster}.
...
Para um cluster $C$, os vértices em $C\backslash \partial C$ são chamados de vértices internos (\textit{internal vertices}). Se $a$ e $b$ são vértices da fronteira de $C$, o caminho $a \cdots b$ se chama caminho do cluster (\textit{cluster path}) e denotamos por $\pi(C)$. Se $a \neq b$, o cluster é chamado de \textit{path cluster}. O cluster $C$ é dito caminho ancestral (\textit{path ancestor}) do cluster $$
