...
Agora, vamos calcular o custo de todos os rebaixamentos, tanto de uma aresta da floresta quanto de uma reserva. As linhas $7$ a $11$ do Programa~\ref{prog:replaceGD} e as linhas $2$ a $7$ do Programa~\ref{prog:find-substitute} são onde ocorrem rebaixamento de alguma aresta. Cada vez que uma aresta é rebaixada, um crédito seu é consumido. Como cada aresta inserida pode ser rebaixada no máximo $\lg n$ vezes, então os $\lg n$ rebaixamentos de uma aresta serão pagos pelos $\lg n$ créditos que restaram dela. Dessa forma, temos que o custo total gerado pelas inserções de $a$ arestas é amortizado $\Oh(a \lg^2 n)$. 
