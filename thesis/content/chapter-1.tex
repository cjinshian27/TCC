...
A rotina de substituição da aresta \texttt{substituaAresta} está descrita no Programa~\ref{prog:replaceGD} abaixo, onde usaremos vários dos métodos auxiliares apresentados nas Seções~\ref{sec:node-edge} e \ref{sec:node-vertex}. Além disso, usaremos um método auxiliar chamado \texttt{rebaixeNívelDaAresta}, mas este será explicado no momento em que estivermos explicando cada trecho do código de \texttt{substituaAresta}.
