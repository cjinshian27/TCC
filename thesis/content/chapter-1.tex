...
Como adotamos a técnica de \textit{Euler tour trees} na implementação das nossas florestas dinâmicas, estamos utilizando uma estrutura chamada \textbf{nó do grafo} que armazena vários atributos relevantes, cuja funcionalidade será descrita mais detalhadamente nas Seções~\ref{sec:code-edge-addition} e \ref{sec:code-edge-removal}. 

Como visto na Seção~\ref{sec:euler-tour-trees}, uma sequência Euleriana é constituída de arestas do tipo \textit{uv} ou vértices do tipo \textit{uu}. Sendo assim, um nó do grafo pode representar tanto um vértice da forma $uu$ (ou simplesmente $u$) quanto uma aresta da forma $uv$, $u \neq v$. Em nossa implementação, estaremos usando um mapa hash \textit{nó} para verificar se a entrada \{$u, v$\} existe ou não, retornando um booleano em tempo esperado $\Oh(1)$.
...
Na Figura~\ref{fig:euler-tour-balanced-tree-is-level-propagation}, note que as arestas $ab$ e $ba$ são a mesma aresta, mas elas existem por conta da sequência Euleriana. Em nossa implementação, para evitar a duplicação do atributo \textit{éNível}, optamos por colocar este atributo em arestas cujas pontas estão em ordem lexicográfica. Então, no nosso exemplo, arestas do tipo $ba$, $da$, $dc$ e $ea$ não tem este atributo definido. Se usássemos números em vez de letras nas pontas das arestas, então o atributo \textit{éNível} existiria apenas em arestas com as pontas lidas em ordem crescente, como $12$, $23$, etc.  
