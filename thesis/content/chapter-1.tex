...
Caso $u$ e $v$ já estejam conectados em $G$, então essa aresta $uv$ é chamada de \textbf{aresta reserva} e ela será armazenada no grafo $R_L$ representado por listas de adjacências, que contfém a seguinte biblioteca:
...
Quando removemos uma aresta da floresta de nível $i$ que quebra uma componente da floresta $F_i$ em duas, note que ao procurar por uma aresta substituta não necessariamente percorreremos todas as arestas reserva em $R_i$. Então nem sempre todas as arestas reserva de $R_i$ incidentes a $T_u$ são rebaixadas, visto que o algoritmo será finalizado no momento em que encontrarmos uma substituta.
...
Note que esta rotina consome $\Oh(\lg^2 n)$ mais o tempo do \texttt{substituaAresta}, que será descrito mais adiante.
...
Voltando ao Programa~\ref{prog:replaceGD}, nas linhas $12$ a $19$, procuramos por uma aresta substituta de nível $j$. Similarmente à linha $7$, a linha $10$ é um laço que terminará quando não existirem mais arestas reserva de nível $j$ (ou seja, quando o contador \textit{arestasReservasDeNível} do nó raiz de $T_u$ estiver nulo) ou quando achamos uma aresta substituta.  

Na linha $13$, acionamos \texttt{procureNóIncideArestaReservaDeNível}, que retornará um nó de vertice $xx$ que incide em alguma aresta reserva de nível $j$. Depois, acionamos \texttt{splay} em $xx$ e atualizamos $T_u$. Na linha $16$, obtemos o vértice $x$ do nó $xx$ e percorremos todos os vizinhos $y$ de de $x$ na lista de adjacências na linha $17$, pois queremos testar se $xy$ é uma aresta substituta.

Para isso, chamamos o método auxiliar \texttt{testeSubstituta} na linha $18$, descrito no Programa~\ref{prog:find-substitute}. Como o nome sugere, o método testa se $xy$ é uma aresta substituta, devolvendo verdadeiro se $xy$ for e falso caso contrário. Quando o método devolve verdadeiro, chegamos à linha $19$ do Programa~\ref{prog:replaceGD}, terminando o algoritmo. Caso contrário, continuamos percorrendo os vizinhos $y$ da lista de adjacências de $x$ e chamando o mesmo método várias vezes.
...
Agora iremos explicar por que a rotina \texttt{substituaAresta} consome tempo amortizado $\Oh(\lg^2 n)$, para um grafo $G$ de $n$ vértices. A amortização ocorre em uma sequência de $a$ inserções e $b$ remoções de arestas. Mostraremos que o custo total destas $a + b$ operações é amortizado $\Oh((a + b) \lg^2 n)$, o que resulta em um custo amortizado por operação de $\Oh(\lg^2 n)$. 

Para cada aresta inserida, ela vai ter um total de $\lg^2 n$ créditos. Como a inserção consome tempo amortizado $\Oh(\lg n)$ em nossa implementação, então $\lg n$  créditos de uma aresta serão descontados do total para realizar a inserção dela, sobrando assim, $\lg n$ créditos.

Agora, vamos calcular o custo de todos os rebaixamentos, tanto de uma aresta da floresta quanto de uma reserva. As linhas $7$ a $11$ do Programa~\ref{prog:replaceGD} e as linhas $2$ a $7$ do Programa~\ref{prog:find-substitute} são onde ocorrem rebaixamento de alguma aresta. Cada vez que uma aresta é rebaixada, um crédito seu é consumido. Como cada aresta inserida pode ser rebaixada em no máximo $\lg n$ vezes, então os $\lg n$ rebaixamentos de uma aresta serão pagos pelos $\lg n$ créditos que restaram dela. Dessa forma, temos que o custo total gerado pelas inserções de $a$ arestas é amortizado $\Oh(a \lg^2 n)$. 

Desconsiderando, então, estas linhas onde ocorrem rebaixamento de aresta, a rotina \texttt{substituaAresta} acaba tendo custo amortizado $\Oh(\lg^2 n)$ nas linhas restantes, e então as $b$ remoções de arestas consomem tempo amortizado $\Oh(b \lg^2 n)$. Dessa forma, concluímos que o custo total da sequência de $a + b$ operações é amortizado $\Oh((a + b)\lg^2 n)$, e assim \texttt{substituaAresta} consome tempo amortizado $\Oh(\lg^2 n)$. Consequentemente, temos que \texttt{removaGD} também consome o mesmo tempo amortizado.