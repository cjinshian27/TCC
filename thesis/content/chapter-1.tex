...
Como estamos utilizando a técnica de \textit{Euler tour trees}, mencionada brevemente na Seção~\ref{sec:dynamic-graph-structure}, então estamos utilizando uma estrutura chamada \textbf{nó} que armazena vários atributos relevantes, cuja funcionalidade será descrita mais detalhadamente nas Seções~\ref{sec:code-edge-addition} e \ref{sec:code-edge-removal}. 
...
Já para o nó do tipo vértice $uu$, iremos armazenar um booleano chamado \textit{incideArestaReservaDeNível} que indica se $uu$ é uma das pontas de alguma aresta reserva com o booleano \textit{éNível} verdadeiro. Dessa forma, se $uv$ é aresta reserva e possui nível $2$, então os vértices $u$ e $v$ em $F_2$ terão \text{incideArestaReservaDeNível} como verdadeiro. Além disso, iremos manter um contador para cada nó do tipo vértice, a fim de calcular a quantidade de vértices em sua subárvore que tenham o booleano \textit{incideArestaDeNível} como verdadeiro. 
...
O consumo de tempo esperado de \texttt{conectadoGD} é O(lg$n$), visto que ele simplesmente chama \texttt{conectadoFD} que tem consumo esperado O(lg$n$). Além disso, a rotina de consulta não altera o nosso grafo, incluindo florestas e listas de adjacências, já que não há nenhuma modificação nele, e isso implica que as invariantes são mantidas. 
