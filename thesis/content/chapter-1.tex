...
Veja que, no Programa~\ref{prog:find-substitute}, as linhas $2$ a $7$ possuem custo amortizado $\Oh(\lg n)$. Isso quer dizer que, enquanto as arestas que estamos testando não forem substitutas, \texttt{testeSubstituta} será acionado várias vezes com esse custo de tempo. No momento em que encontrarmos uma substituta, o método \texttt{testeSubstituta} será acionado uma única vez e consumirá tempo amortizado $\Oh(\lg^2 n)$ por causa das linhas 10 e 11, e assim o algoritmo será finalizado. 
...
Agora mostraremos que, se ocorreram $m$ operações de inserção e remoção de arestas desde a criação do grafo, então o custo total de tal sequência de operações é $\Oh(m \lg^2 n)$, o que resulta em um custo amortizado por operação de $\Oh(\lg^2 n)$.
...
Isso quer dizer que a inserção da aresta $e$ vai pagar por cada execução das linhas $8$ a $11$ do Programa~\ref{prog:replaceGD} e das linhas $2$ a $7$ do Programa~\ref{prog:find-substitute} que processa a aresta $e$. Como a inserção custa $\Oh(\lg n)$ e essas linhas custam $\Oh(\lg n)$ e são executadas $\Oh(\lg n)$ vezes, pois $e$ pode ser rebaixada no máximo $\left\lceil \lg n \right\rceil$ vezes, o custo pago por uma inserção é $\Oh(\lg^2 n)$.

Já uma remoção de aresta, executada pelo Programa~\ref{prog:removeGD-version2}, custa $\Oh(\lg^2 n)$ mais o custo do \texttt{substituaAresta}. O custo do \texttt{substituaAresta} é $\Oh(\lg^2 n)$ excluindo-se as execuções das linhas $7$ a $11$ do Programa~\ref{prog:replaceGD}, como também as linhas $2$ a $7$ do Programa~\ref{prog:find-substitute}. Isso porque, desconsiderando estas linhas onde ocorrem rebaixamento de aresta, na linha $2$ do Programa~\ref{prog:replaceGD} acaba tendo custo $\Oh(\lg n)$ nas linhas restantes. Apesar de as linhas $10$ e $11$ do Programa~\ref{prog:find-substitute} consumirem $\Oh(\lg^2 n)$, ela será executada uma única vez quando encontrarmos a substituta e o algoritmo será finalizado. Assim, como a linha $2$ pode ser executada em no máximo $\Oh(\lg n)$ vezes, temos que \texttt{substituaAresta} consome tempo amortizado $\Oh(\lg^2 n)$ por operação de remoção.

Com isso, concluímos que o custo total da sequência de $m$ inserções e remoções é $\Oh(m \lg^2 n)$, e assim cada inserção e remoção consome tempo amortizado $\Oh(\lg^2 n)$.