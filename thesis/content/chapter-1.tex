...
\end{programruledcaption}
 ...
 Em ambas as rotinas auxiliares, o método \texttt{puxeParaRaiz} consome tempo amortizado $\Oh(\lg n)$, pois o levar um nó até a raiz da árvore é proporcional à altura da árvore. Já \texttt{atualizeIncideArestaReservaDeNível} consome tempo $\Oh(1)$. Dessa forma, ambas as rotinas \texttt{decrementeArestasReservasDeNível} e \texttt{incrementeArestasReservasDeNível} consomem tempo amortizado $\Oh(\lg n)$ em nossa implementação.
Como tais rotinas auxiliares não fazem nenhuma modificação no grafo, então todas as invariantes são mantidas. 
