...
Consequentemente, temos que $G_1 \subseteq G_2 \subseteq \cdots \subseteq G_{\left\lceil \lg n \right\rceil}$, para um grafo de $n$ vértices. Daqui, deduzimos que $G = G_{\left\lceil \lg n \right\rceil}$ e que $F_{\left\lceil \lg n \right\rceil}$ é uma floresta maximal de $G$. Dessa maneira, sempre que estivermos realizando alguma operação de alteração ou consulta de conexidade em nosso grafo $G$, estamos também a realizando na floresta $F_{\left\lceil \lg n \right\rceil}$ de $G$. Da mesma forma, quando chamamos o construtor do grafo dinâmico, estamos criando $\left\lceil \lg n \right\rceil$ florestas de vértices isolados.
...
Na Seção~\ref{sec:dynamic-graph-implementation}, iremos descrever como as rotinas da biblioteca funcionam e como cada uma preserva as invariantes acima, de modo a manter a corretude do algoritmo durante toda a sua execução.

Por fim, para simplificar um pouco a notação, iremos escrever $L = \left\lceil \lg n \right\rceil$, isto é, $F_{\left\lceil \lg n \right\rceil}$ passa a ser escrito como $F_{L}$, da mesma forma que $G_L = G_{\left\lceil \lg n \right\rceil}$ e $R_L = R_{\left\lceil \lg n \right\rceil}$.
...
A nossa implementação \cite{chung2025} de listas de adjacências possui um custo O($n$) ao acionar o construtor \texttt{listaDeAdjacências}, e para 
...
Como a inserção de arestas em uma floresta dinâmica com $n$ vértices consome tempo O(lg$n$) (amortizado, na nossa implementação), então temos que \texttt{adicioneGD} também terá o seu custo amortizado de tempo O(lg$n$). 

\section{Remoção de arestas no grafo dinâmico}
\label{sec:dynamic-graph-edge-removal}

A remoção de arestas se divide em dois casos: remoção de uma aresta reserva ou remoção de uma aresta da floresta $F_{\left\lceil \lg n \right\rceil}$.
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \draw[edge] (u) -- (v) node[midway, below] {};
    ...
    \end{tikzpicture}
    \caption{Um grafo com 10 vértices. As arestas pretas são da floresta maximal $F_{\left\lceil \lg n \right\rceil}$ do grafo, enquanto as vermelhas são arestas reservas.}
    \label{fig:graph_with_Tu_and_Tv}
\end{figure}

Assim, torna-se necessário uma implementação mais eficiente para a rotina de remoção de arestas, e por isso apresentaremos a solução de Holm, de Lichtenberg e Thorup~\cite{jacob_holm}, que resolve este problema em tempo amortizado O(lg$^2n$).

Por fim, o custo da rotina \texttt{grafoDinâmico(G, n)} será O($n$lg$n$), enquanto que, na nossa implementação, o custo amortizado das demais rotinas será O(lg$^2n$), que serão descritas na Seção~\ref{sec:dynamic-graph-implementation}.
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{As arestas pretas são da floresta maximal $F_{\left\lceil \lg n \right\rceil}$ do grafo, enquanto as vermelhas são reservas. A aresta $uv$ tracejada está prestes a ser removida e substituída pela $ad$ ou $cv$, que ligam $T_u$ e $T_v$.}
...
Um \textbf{nó} pode representar tanto um vértice da forma $uu$ (ou simplesmente $u$) quanto uma aresta da forma $uv$, $u \neq v$. Em nossa implementação, estaremos usando um mapa hash \textit{nó} para verificar se a entrada \{$u, v$\} existe ou não, retornando um booleano em tempo esperado O($1$).
...
Como o consumo de tempo de \texttt{conectadosFD}, \texttt{adicioneFD} e de \texttt{incrementeArestasReservasDeNível} é amortizado O(lg$n$), e \texttt{adicioneLA} possui custo esperado O($1$), então temos que \texttt{adicioneGD} possui consumo de tempo amortizado O(lg$n$).
...
\begin{figure}
    ...
    \begin{minipage}[c]{0.8\textwidth}
        \centering
        \begin{tikzpicture}
        [scale=0.65, node/.style={circle,draw,minimum size=2em, thick, font=\large},
        edge/.style={thick, black},
        reserve/.style={red, thick},
        removed/.style={black, thick, dashed}]
