...
Isso quer dizer que a inserção da aresta $e$ vai pagar por cada execução das linhas $7$ a $11$ do Programa~\ref{prog:replaceGD} e das linhas $2$ a $7$ do Programa~\ref{prog:find-substitute} que processa a aresta $e$. Como a inserção custa $\Oh(\lg n)$ e essas linhas custam $\Oh(\lg n)$ e são executadas $\Oh(\lg n)$ vezes, pois $e$ pode ser rebaixada no máximo $\left\lceil \lg n \right\rceil$ vezes, o custo pago por uma inserção é $\Oh(\lg^2 n)$.
