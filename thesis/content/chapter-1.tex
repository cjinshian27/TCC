...
Além disso, falta apresentar a estratégia usada para percorrer as arestas da floresta e reservas de forma eficiente. Isso também será descrito na próxima seção.
...
Assim, Henzinger e King \cite{henzinger_king} propuseram representar uma floresta por uma coleção de árvores Eulerianas, pois assim as operações de consulta de conexidade e de alteração na árvore possuem consumo esperado $\Oh(\lg n)$ (amortizado em nossa implementação).
...
Sabemos, então, que o valor de \textit{éNível} sofre alteração quando ocorre rebaixamento de alguma aresta. Assim, para cada nó (seja ele vértice ou aresta) introduzimos um contador chamado \textit{arestasDeNível}, que armazena a quantidade de arestas com o \textit{éNível} verdadeiro em sua subárvore. A rotina auxiliar que atualiza este contador está descrita no Programa~\ref{prog:updateIsLevelCount-GD}.
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Uma floresta dinâmica $F_i$ onde nós pintados em vermelho indicam que são arestas com o éNível verdadeiro. Isso quer dizer que todas as arestas da floresta são de nível $i$ em nosso exemplo. Embaixo de cada nó temos o contador arestasDeŃível. Note que o campo éNível não existe em vértices, somente em arestas.}
...
Diferente de \textit{éNível}, o campo \textit{incideArestaReservaDeNível} de um vértice pode ficar mudando de valor quando houver inserções e remoções de arestas reservas no grafo. Nesse sentido, podemos introduzir dois métodos que modificam o atributo \textit{incideArestaReservaDeNível} e atualizam o contador \textit{arestasReservasDeNível}.
