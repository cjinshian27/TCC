...
Desconsiderando, então, estas linhas onde ocorrem rebaixamento de aresta, a rotina \texttt{substituaAresta} acaba tendo custo amortizado $\Oh(\lg^2 n)$ nas linhas restantes, e então as $b$ remoções de arestas consomem tempo amortizado $\Oh(b \lg^2 n)$. Dessa forma, o custo total da sequência de $a + b$ operações é $\Oh((a + b)\lg^2 n)$, e assim \texttt{substituaAresta} consome tempo amortizado $\Oh(\lg^2 n)$ por operação de remoção. Consequentemente, temos que \texttt{removaGD} também consome o mesmo tempo amortizado.