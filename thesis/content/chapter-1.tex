...
Para criar o grafo dinâmico $G$ de $n$ vértices, chamamos a rotina \texttt{grafoDinâmico(G, n)}, que por sua vez irá chamar \texttt{florestaDinâmica(F, n)}, inicializando $\left\lceil \lg n \right\rceil$ florestas dinâmicas de $n$ vértices isolados, e \texttt{listaDeAdjacências(R, n)}, inicializando $\left\lceil \lg n \right\rceil$ listas de adjacências de $n$ vértices isolados. Como as duas últimas rotinas consomem tempo $\Oh(n)$, então \texttt{grafoDinâmico} consome tempo $\Oh(n \lg n)$.
...
Como explicado na Seção~\ref{sec:dynamic-graph-edge-types}, quando inserimos uma aresta $uv$ no grafo $G$ chamando a rotina \texttt{adicioneGD(G, u, v)}, ela pode ser tanto uma aresta reserva quanto uma aresta da floresta. Assumindo que não exista nenhuma aresta $uv$ na floresta maximal $F_L$ de $G$, temos dois cenários:
...
Assim, torna-se necessário uma implementação mais eficiente para a rotina de remoção de arestas, e por isso apresentaremos a solução de Holm, de Lichtenberg e Thorup~\cite{jacob_holm}, que resolve este problema em tempo amortizado $\Oh(n \lg 2^n)$.

Por fim, o custo da rotina \texttt{grafoDinâmico(G, n)} será $\Oh(n \lg n)$, enquanto que, na nossa implementação, o custo amortizado das demais rotinas será $\Oh(n \lg 2^n)$, que serão descritas na Seção~\ref{sec:dynamic-graph-implementation}.
