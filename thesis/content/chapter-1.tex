...
\begin{programruledcaption}{\texttt{atualizeArestasDeNível($F$, $u$, $v$)} \label{prog:updateIsLevelCount-GD}}
    ...
    \textbf{se} arestaUV.filhoEsq $\neq$ \texttt{NIL} \textbf{então}
        \textbf{se} arestaUV.filhoEsq.éNível \textbf{então}
            c := c + 1
    \textbf{se} arestaUV.filhoDir $\neq$ \texttt{NIL} \textbf{então}
        \textbf{se} arestaUV.filhoDir.éNível \textbf{então}
            c := c + 1
...
Na Figura~\ref{fig:euler-tour-balanced-tree-is-level-propagation}, note que os nós $\emph{ab}$ e $\emph{ba}$ representam a mesma aresta. Assim, para evitar a duplicação do atributo \textit{éNível}, optamos por colocar este atributo como verdadeiro nos nós de aresta cujos vértices estão em ordem lexicográfica. Então, no nosso exemplo, nós do tipo $\emph{ba}$, $\emph{da}$, $\emph{dc}$ e $\emph{ea}$ vão ter este atributo falso. 

Note que o Programa~\ref{prog:updateIsLevelCount-GD} consome tempo constante, pois envolve apenas atribuições de variáveis e comparações.
...
\begin{programruledcaption}{\texttt{atualizeArestasReservasDeNível($F$, $u$)} \label{prog:updateReserveEdgesCount-GD}}
    ...
    \textbf{se} vérticeU.filhoEsq $\neq$ \texttt{NIL} \textbf{então}
        \textbf{se} vérticeU.filhoEsq.incideArestaReservaDeNível  \textbf{então}
            c := c + 1
    \textbf{se} vérticeU.filhoDir $\neq$ \texttt{NIL} \textbf{então}
        \textbf{se} vérticeU.filhoDir.incideArestaReservaDeNível  \textbf{então}
            c := c + 1
...
Note que o Programa~\ref{prog:updateReserveEdgesCount-GD} consome tempo constante $\Oh(1)$, pois envolve apenas atribuições de variáveis e comparações de igualdade.
