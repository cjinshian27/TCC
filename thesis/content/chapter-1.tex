...
Já uma remoção de aresta, executada pelo Programa~\ref{prog:removeGD-version2}, custa $\Oh(\lg^2 n)$ mais o custo do \texttt{substituaAresta}. O custo do \texttt{substituaAresta} é $\Oh(\lg^2 n)$ excluindo-se as execuções das linhas $7$ a $10$ do Programa~\ref{prog:replaceGD}, assim como as linhas $4$ a $9$ do Programa~\ref{prog:find-substitute}. Desconsiderando estas linhas onde ocorrem rebaixamentos de arestas, a finalização de cada execução da linha $2$ do Programa~\ref{prog:replaceGD} possui custo $\Oh(\lg n)$ nas linhas restantes. 

Além disso, apesar das linhas $12$ e $13$ do Programa~\ref{prog:find-substitute} consumirem tempo $\Oh(\lg^2 n)$, tais linhas serão executadas uma única vez quando encontrarmos uma substituta e o algoritmo termina. Assim, como a linha $2$ do Programa~\ref{prog:replaceGD} pode ser executada no máximo $\Oh(\lg n)$ vezes, temos que \texttt{substituaAresta} consome tempo amortizado $\Oh(\lg^2 n)$ por operação de remoção.
