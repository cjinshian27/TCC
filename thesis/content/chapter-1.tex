...
Em nossa implementação, quando inserimos uma aresta $uv$ na floresta $F_L$ usando o \texttt{adicioneGD}, por padrão o \textit{éNível} de $uv$ é falso. Por isso, precisamos de um método \texttt{atualizeÉNível}, que irá atualizar tal atributo de um nó de aresta para verdadeiro. Este método está descrito no Programa~\ref{prog:updateIsLevel-GD}.
...
O Programa~\ref{prog:updateIsLevelCount-GD} consome tempo $\Oh(\lg n)$ e Programa~\ref{prog:updateIsLevelCount-GD-splay} consome tempo amortizado $\Oh(\lg n)$. Já o Programa~\ref{prog:updateIsLevel-GD}, em nossa implementação, consome tempo amortizado $\Oh(\lg n)$, já que ele aciona o Programa~\ref{prog:updateIsLevelCount-GD-splay}. Veja que estes três métodos não alteram alguma floresta do grafo e, portanto, todas as três invariantes são preservadas.
