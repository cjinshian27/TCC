...
Já uma remoção de aresta, executada pelo Programa~\ref{prog:removeGD-version2}, custa $\Oh(\lg^2 n)$ mais o custo do \texttt{substituaAresta}. O custo do \texttt{substituaAresta} é $\Oh(\lg^2 n)$ excluindo-se as execuções das linhas $7$ a $10$ do Programa~\ref{prog:replaceGD}, assim como as linhas $4$ a $9$ do Programa~\ref{prog:find-substitute}. Desconsiderando estas linhas onde ocorrem rebaixamentos de arestas, cada iteração do para da linha $2$ do Programa~\ref{prog:replaceGD} possui custo $\Oh(\lg n)$ enquanto uma aresta substituta não for encontrada, ou seja, excetuando as linhas $10$ a $17$ do Programa~\ref{prog:find-substitute}. 

No momento em que encontrarmos uma substituta, as linhas $10$ a $17$ do Programa~\ref{prog:find-substitute} serão executadas uma única vez e consumirão tempo $\Oh(\lg^2 n)$ devido às linhas $12$ e $13$, terminando o algoritmo logo em seguida. Assim, como a linha $2$ do Programa~\ref{prog:replaceGD} pode ser executada no máximo $\Oh(\lg n)$ vezes, temos que \texttt{substituaAresta} consome tempo amortizado $\Oh(\lg^2 n)$ por operação de remoção.
