...
Já uma remoção de aresta, executada pelo Programa~\ref{prog:removeGD-version2}, custa $\Oh(\lg^2 n)$ mais o custo do \texttt{substituaAresta}. O custo do \texttt{substituaAresta} é $\Oh(\lg^2 n)$ excluindo-se as execuções das linhas $7$ a $11$ do Programa~\ref{prog:replaceGD}, como também as linhas $2$ a $7$ do Programa~\ref{prog:find-substitute}. Isso porque, desconsiderando estas linhas onde ocorrem rebaixamento de aresta, na linha $2$ do Programa~\ref{prog:replaceGD} acaba tendo custo $\Oh(\lg n)$ nas linhas restantes. Apesar de as linhas $10$ e $11$ do Programa~\ref{prog:find-substitute} consumirem $\Oh(\lg^2 n)$, este trecho do código será executado uma única vez quando encontrarmos a substituta e o algoritmo será finalizado. Assim, como a linha $2$ pode ser executada em no máximo $\Oh(\lg n)$ vezes, temos que \texttt{substituaAresta} consome tempo amortizado $\Oh(\lg^2 n)$ por operação de remoção.
