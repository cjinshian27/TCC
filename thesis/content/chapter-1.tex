...
\begin{programruledcaption}{\texttt{adicioneGD($G$, $u$, $v$)} \label{prog:addGD-version2}}
    ...
    \textbf{senão}
        \texttt{adicioneFD($G.F_{\left\lceil \lg n \right\rceil}$, $u$, $v$)}
        \texttt{atualizeÉNível($F_{\left\lceil \lg n \right\rceil}$, $u$, $v$, $\textbf{verdadeiro}$)}
    \end{lstlisting}
    \vspace{-0.5\baselineskip}
\end{programruledcaption}
            ...
            \textbf{para} $y \in G.R[x]$ \textbf{faça}
                ...
                \textbf{senão}
                    ...
                    \textbf{se} x > y \textbf{então}
                        x $\leftrightarrow$ y
                    \texttt{atualizeÉNível($F_j$, $x$, $y$, $\textbf{verdadeiro}$)}
                    \textbf{devolva}
    \end{lstlisting}
    \vspace{-0.5\baselineskip}
\end{programruledcaption}
