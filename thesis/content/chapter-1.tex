...
Para criar um grafo dinâmico $G$ com $n$ vértices e inicialmente sem arestas, primeiro armazenamos o nível máximo do grafo, no caso o valor de $\left\lceil \lg n \right\rceil$, numa variável chamada $L$, onde podemos extrair e usar o valor do nível máximo chamando $G.L$. Em seguida, chamamos a rotina \texttt{grafoDinâmico($n$)}, que por sua vez chamará \texttt{florestaDinâmica($n$)}, criando $L$ florestas dinâmicas com $n$ vértices, e \texttt{listasDeAdjacências($n$)}, criando $L$ grafos com $n$ vértices representados por listas de adjacências. 
...
\begin{programruledcaption}{\texttt{grafoDinâmico($n$)} \label{prog:newGD}}
    ...
    G.L := $\left\lceil \lg n \right\rceil$ 
    \textbf{para} $i$ := $1$ \textbf{até} G.L \textbf{faça}
...
Como representamos uma Euler tour tree por uma árvore binária de busca, cada nó possui apontadores para o filho esquerdo, filho direito e o pai do p. Em descrição de nossa implementação, para um nó $p$, denotamos tais apontadores por \textit{p.esq}, \textit{p.dir} e \textit{p.pai}, respectivamente. O motivo de usar um apontador para o pai do nó será discutido depois. A seguir, descreveremos a funcionalidade de cada tipo de nó, bem como outros atributos relevantes que lhe pertencem. 
...
Em nossa implementação, quando inserimos uma aresta $uv$ na floresta $F_L$ usando o \texttt{adicioneGD}, por padrão o \textit{éNível} de $uv$ é falso. Por isso, precisamos de um método \texttt{atualizeÉNível}, que irá manter o atributo \textit{éNível} com o valor correto. Assim, ao inserirmos $uv$ em $F_L$, este método atualizará tal atributo deste nó de aresta para verdadeiro. Quando rebaixamos tal aresta, precisamos atualizar este seu atributo para falso em $F_L$, pois isso indica que $uv$ deixou de ser de nível $L$. Este método está descrito no Programa~\ref{prog:updateIsLevel-GD}.

\begin{programruledcaption}{\texttt{atualizeÉNível($F$, $u$, $v$, $b$)} \label{prog:updateIsLevel-GD}}
    \noindent\textbf{Entrada}: Recebe uma floresta $F$, vértices $u$ e $v$ e um booleano $b$.\\
    \noindent\textbf{Efeito}: Atualiza o atributo \textit{éNível} de $uv$ e o contador \textit{arestasDeNível}.
    \vspace{-0.5\baselineskip}
    \begin{lstlisting}[
        language={[brazilian]pseudocode},
        style=pseudocode,
...
\begin{programruledcaption}{\texttt{atualizeArestasDeNível($\emph{p}$)} \label{prog:updateIsLevelCount-GD}}
    ...
    \noindent\textbf{Efeito}: Atualiza o contador \textit{arestasDeNível} do p.
    \vspace{-0.5\baselineskip}
    \begin{lstlisting}[
        language={[brazilian]pseudocode},
        style=pseudocode,
...
Na nossa implementação, a atualização deste contador é feito na operação \textit{splay} sempre que essa executa alguma rotação. Além disso, para alterar o atributo \textit{éNível} de um nó de aresta, acionamos a rotina do Programa~\ref{prog:updateIsLevel-GD}, que por sua vez usa a rotina do Programa~\ref{prog:updateIsLevelCount-GD}.

O Programa~\ref{prog:updateIsLevel-GD} consome tempo amortizado $\Oh(\lg n)$. Veja que este método não altera a floresta $F_i$, altera apenas uma das árvores binárias que a representam. Portanto, todas as três invariantes são preservadas.
...
Cada nó das florestas também guardará um contador \textit{arestasReservasDeNível}, que armazena a quantidade de nós em sua subárvore com o atributo \textit{incideArestaReservaDeNível} verdadeiro. Esse contador deve ser mantido atualizado quando é feita qualquer alteração em uma das árvores binárias que representam as florestas. Na nossa implementação, isso é feito na rotina \textit{splay}, sempre que essa executa uma rotação. 

O método que mantém este contador se chama \texttt{atualizeArestasReservasDeNível} e está descrito no Programa~\ref{prog:updateReserveEdgesCount-GD}. 
...
O Programa~\ref{prog:updateReserveEdgesCount-GD} consome tempo $\Oh(\lg n)$ em nossa implementação por conta das operações \textit{splay}. Como este método não altera a floresta $F_i$, altera apenas uma da árvores uma das árvores binárias que a representam, então as três invariantes são preservadas.
...
Para resolver este problema de maneira eficiente, introduzimos um método auxiliar chamado \texttt{procureNóIncideArestaReservaDeNível}, que procurará um vértice que incide em uma das arestas reserva de nível $i$.
Essa rotina devolve um vértice $u$ que incide a uma das arestas reserva de nível $i$. Assim, podemos percorrer em $R_i$ cada vizinho $v$ de $u$ para verificar se $uv$ liga $T_u$ a $T_v$. 
...
O campo \textit{incideArestaReservaDeNível} de um nó de vértice $u$ pode mudar de valor quando houver inserções e remoções de arestas reserva que possuem como uma da suas pontas o vértice $u$. Portanto, mostraremos dois métodos que modificam este campo e atualizam o contador \textit{arestasReservasDeNível}. 
...
\begin{programruledcaption}{\texttt{adicioneGD($G$, $u$, $v$)} \label{prog:addGD-version2}}
    ...
    \textbf{senão}
        \texttt{adicioneFD($G.F_{\left\lceil \lg n \right\rceil}$, $u$, $v$)}
        \texttt{atualizeÉNível($G.F_{\left\lceil \lg n \right\rceil}$, $u$, $v$, $\textbf{verdadeiro}$)}
    \end{lstlisting}
    \vspace{-0.5\baselineskip}
\end{programruledcaption}
...
Atualizamos os atributos desses nós que descrevemos para fazermos a remoção eficiente de arestas, cuja rotina será descrita também com ajustes na Seção~\ref{sec:code-edge-removal-second-version}. Note que o método \texttt{adicioneGD} continua tendo o mesmo consumo de tempo após esses ajustes. 
...
\begin{programruledcaption}{\texttt{rebaixeNívelDaAresta($G$, $p$, $j$)} \label{prog:decrease-edge-level}}
    \noindent\textbf{Entrada}: Recebe o grafo $G$, um nó de aresta $p$ e o nível $j$.\\
...
No Programa~\ref{prog:decrease-edge-level}, atualizamos o nível do nó de aresta $xy$ de $j$ para $j-1$. As assinaturas $\emph{p[1]}$ e $\emph{p[2]}$ retornam os valores das duas pontas do nó $p$ (nesse caso de $xy$), que no nosso exemplo atribuímos às variáveis $x$ e $y$. Ademais,  como rebaixamos $xy$ de $F_j$ para $F_{j - 1}$, então em $F_j$ atualizamos o seu atributo \textit{éNível} para falso, e em $F_{j-1}$ atualizamos este atributo para verdadeiro. Veja que \texttt{rebaixeNívelDaAresta} consome tempo amortizado $O(\lg n)$.

Voltando ao Programa~\ref{prog:replaceGD}, as linhas $10$ a $26$ procuramos por uma aresta substituta de nível $j$. Similarmente à linha $7$, a linha $10$ é um laço \textit{while} que terminará quando não existir mais arestas reserva de nível $j$ (ou seja, o contador \textit{arestasReservasDeNível} do nó raiz de $T_u$ é zerado) ou quando achamos uma aresta substituta.  

Na linha $11$, acionamos \texttt{procureNóIncideArestaReservaDeNível}, que retornará um nó de vertice $xx$ que incide uma das arestas reserva de nível $i$. Como tanto $\emph{nó[1]}$ quanto $\emph{nó[2]}$ retornam $x$ para um nó de vértice $xx$, escolhemos o primeiro para podermos percorrer todos os vizinhos $y$ de lista de adjacências de $x$ na linha $13$, pois queremos testar se $xy$ é uma potencial aresta substituta.
...
Veja que~\texttt{conectadosGD(G, x, y)} só devolverá falso quando $x$ está em $T_u$ e $y$ está em $T_v$, pois assim os dois vértices estariam em componentes separadas da floresta. Este caso, abordado nas linhas $20$ a $26$ do Programa~\ref{prog:replaceGD}, mostra que encontramos $xy$ como uma aresta substituta, e finalizamos o algoritmo conectando $x$ de $T_u$ a $y$ de $T_v$ de todas as florestas $F_k$, para $k = j, \ldots, L$, por conta da invariante~\ref{invariant2}. Além disso, se a aresta substituta encontrada é de nível $j$, precisamos atualizar o atributo \textit{éNível} do nó desta aresta para verdadeiro quando a adicionamos na floresta $F_j$, como acontece na linha $25$.
