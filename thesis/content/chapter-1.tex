...
Veja que~\texttt{conectadosGD(G, x, y)} só devolverá falso quando $x$ está em $T_u$ e $y$ está em $T_v$, pois assim os dois vértices estariam em componentes separadas da floresta $F_j$. Este caso, abordado nas linhas $8$ a $15$ do Programa~\ref{prog:find-substitute}, mostra que encontramos $xy$ como uma aresta substituta, e finalizamos o algoritmo incluindo $xy$ em todas as florestas $F_k$, para $k = j, \ldots, L$, para manter a invariante~\ref{invariant2}. Além disso, se a aresta substituta encontrada é de nível $j$, precisamos atualizar o atributo \textit{éNível} do nó desta aresta em $F_j$ para verdadeiro quando a adicionamos na floresta $F_j$, como acontece na linha $14$. Por fim, devolvemos verdadeiro pois, neste caso, achamos uma aresta substituta. Veja que o Programa~\ref{prog:find-substitute} tem custo $\Oh(\lg^2 n)$ por causa das linhas 10 e 11. 

Agora, explicaremos o custo da rotina \texttt{substituaAresta}. No pior caso, uma execução desta rotina pode consumir muito tempo. Por exemplo, se o grafo já está com $m = \Theta(n^2)$ arestas inseridas, todas de nível $L$, pode ocorrer uma remoção que aciona o \texttt{substituaAresta} e que acarreta o rebaixamento de $\Oh(n^2)$ arestas, a um custo $\Theta(n^2 \lg n)$. 

No entanto, para chegar a essa situação, teriam ocorrido $\Oh(n^2)$ inserções, cada uma com um custo bem mais barato, de $\Oh(\lg n)$. Isso sugere que possivelmente uma análise amortizada do custo das operações leva a um custo por operação mais baixo.

Agora iremos explicar por que a rotina \texttt{substituaAresta} consome tempo amortizado $\Oh(\lg^2 n)$, para um grafo $G$ de $n$ vértices. A amortização ocorre em uma sequência de $a$ inserções e $b$ remoções de arestas. Mostraremos que o custo total destas $a + b$ operações é $\Oh((a + b) \lg^2 n)$, o que resulta em um custo amortizado por operação de $\Oh(\lg^2 n)$. 
...
Isso quer dizer que a inserção da aresta $e$ vai pagar por cada execução das linhas $8$ a $11$ do Programa~\ref{prog:replaceGD} e das linhas $3$ a $7$ do Programa~\ref{prog:decrease-edge-level} que processa a aresta $e$. Como a inserção custa $\Oh(\lg n)$ e essas linhas custam $\Oh(\lg n)$ e são executadas $\Oh(\lg n)$ vezes, pois $e$ pode ser rebaixada no máximo $\left\lceil \lg n \right\rceil$ vezes, o custo total amortizado de uma inserção é $\Oh(\lg^2 n)$. Dessa forma, temos que o custo total gerado pelas inserções de $a$ arestas é amortizado $\Oh(a \lg^2 n)$. 

Desconsiderando, então, estas linhas onde ocorrem rebaixamento de aresta, na linha $2$ do Programa~\ref{prog:replaceGD}, cada iteração  acaba tendo custo amortizado $\Oh(\lg n)$ nas linhas restantes. Como a linha $2$ pode ser executada em no máximo $\Oh(\lg n)$ vezes, então temos que \texttt{substituaAresta} possui tempo amortizado $\Oh(\lg^2 n)$ por operação de remoção. Sendo assim, as $b$ remoções de arestas consomem tempo amortizado $\Oh(b \lg^2 n)$. 

Dessa forma, concluímos que o custo total da sequência de $a + b$ operações é  $\Oh((a + b)\lg^2 n)$, e assim \texttt{substituaAresta} consome tempo amortizado $\Oh(\lg^2 n)$ por operação. Consequentemente, temos que \texttt{removaGD} também consome o mesmo tempo amortizado por operação de remoção.