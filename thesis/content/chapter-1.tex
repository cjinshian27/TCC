...
\begin{programruledcaption}{\texttt{atualizeÉNível($p$, $b$)} \label{prog:updateIsLevel-GD}}
    \noindent\textbf{Entrada}: Recebe um nó de aresta $p$ e um booleano $b$.\\
    ...
    \texttt{splay($p$)}
    p.éNível := b
    \texttt{atualizeArestasDeNível($p$)}
\end{lstlisting}
\vspace{-0.5\baselineskip}
\end{programruledcaption}
