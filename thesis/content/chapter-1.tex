...
Em particular, essa rotina deve percorrer as arestas da floresta e as arestas reservas de maneira eficiente. Apresentaremos a estratégia usada para isso nas Seções~\ref{sec:node-edge}~e~\ref{sec:node-vertex}.
...
Como representamos uma Euler tour tree por uma árvore binária de busca, cada nó possui apontadores para o filho esquerdo, filho direito e o pai do nó. Em descrição de nossa implementação, para um nó $p$, denotamos tais apontadores por \textit{p.esq}, \textit{p.dir} e \textit{p.pai}, respectivamente. O motivo de usar um apontador para o pai do nó será discutido depois. A seguir, descreveremos a funcionalidade de cada tipo de nó, bem como outros atributos relevantes que lhe pertencem. 
...
\begin{programruledcaption}{\texttt{atualizeArestasDeNível($\emph{nó}$)} \label{prog:updateIsLevelCount-GD}}
    ...
    \noindent\textbf{Efeito}: Atualiza o contador \textit{arestasDeNível} do nó.
    \vspace{-0.5\baselineskip}
    \begin{lstlisting}[
        language={[brazilian]pseudocode},
        style=pseudocode,
