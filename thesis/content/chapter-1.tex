...
O Programa~\ref{prog:updateReserveEdgesCount-GD} consome tempo $\Oh(1)$ em nossa implementação por conta das operações \textit{splay}. Como este método não altera a floresta $F_i$, altera apenas uma da árvores uma das árvores binárias que a representam, então as três invariantes são preservadas.
