...
Ainda na mesma imagem, a aresta \textit{bc} está prestes a ser removida, o que acabaria quebrando uma componente da floresta $F_i$ em $T_u'$ e $T_v'$. Veja que, para reconectar $T_u'$ e $T_v'$, percorreríamos as seguintes arestas reserva, nesta ordem: $ub$,  $uc$ e $ac$. Veja que, embora $ac$ seja uma substituta que reconecte $T_u'$ e $T_v'$, a $uc$ seria escolhida em seu lugar por ser a de menor peso, e está em $R_i$. 
...
\begin{figure}[H]
    ...
    \begin{minipage}[c]{0.8\textwidth}
        ...
        \end{tikzpicture}
    \end{minipage}
    \caption{Representação do grafo com a aresta substituta $ad$ por ser a de menor peso em $R_4$ que conecta $T_u$ a $T_v$, tornando-se uma aresta da floresta $F_4$. A próxima aresta a ser removida é \textit{bc}, que está tracejada, acaba de quebrar uma componente de $F_i$ em $T_u'$ e $T_v'$.}
    \vspace{-1cm}
...
No caso das arestas reserva com uma ponta incidente a $T_u$ e outra em $T_v$, nenhuma chega a ser rebaixada, visto que a de menor peso já se tornaria uma aresta da floresta e as outras nem seriam percorridas. Por fim, as arestas reserva com ambas as pontas em $T_v$ não são afetadas visto que nenhuma delas são percorridas durante a busca por uma substituta.
...
Já uma remoção de aresta, executada pelo Programa~\ref{prog:removeGD-version-msf-final-version}, custa $\Oh(\lg^2 n)$ mais o custo do \texttt{substituaArestaMSF}. O custo do \texttt{substituaArestaMSF} é $\Oh(\lg^2 n)$ excluindo-se as execuções das linhas $7$ a $10$ do Programa~\ref{prog:replaceMSF}, assim como as linhas $7$ a $14$ do Programa~\ref{prog:find-substitute-MSF}. Desconsiderando estas linhas onde ocorrem rebaixamentos de arestas, a finalização de cada execução da linha $2$ do Programa~\ref{prog:replaceMSF} possui custo $\Oh(\lg n)$ nas linhas restantes.

Além disso, apesar das linhas $18$ e $19$ do Programa~\ref{prog:find-substitute-MSF} consumirem tempo $\Oh(\lg^2 n)$, tais linhas serão executadas uma única vez quando encontrarmos uma substituta e o algoritmo termina. Assim, como a linha $2$ do Programa~\ref{prog:replaceMSF} pode ser executada no máximo $\Oh(\lg n)$ vezes, temos que \texttt{substituaAresta} consome tempo amortizado $\Oh(\lg^2 n)$ por operação de remoção.
