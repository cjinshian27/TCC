...
\begin{itemize}
    ...
    \item \texttt{\textbf{extraiaMinLAMSF(R, u)}}: remove e retorna o par ($v$, $w$), onde $v$ é vértice do min-heap de $u$ em $R$ com chave mínima $w$.
\end{itemize} 

Uma chamada à rotina \texttt{adicioneLAMSF(R, u, v, w)} adiciona o par ($u$, $w$) no min-heap de $v$, como também adiciona o par ($v$, $w$) no min-heap de $u$, consumindo tempo $\Oh(\lg n)$. Similarmente, uma chamada à rotina \texttt{removaLAMSF(R, u, v)} remove o par ($u$,~$w$) do min-heap de $v$, como também remove o par ($v$, $w$) do min-heap de $u$, consumindo também tempo $\Oh(\lg n)$. 
...
\begin{programruledcaption}{\texttt{atualizePesoMínimo($R$, $\emph{p}$)} \label{prog:updateMinWeight}}
    \noindent\textbf{Entrada}: Recebe um nó $w$ e as listas de adjacências $R$.\\
    \noindent\textbf{Efeito}: Atualiza o atributo \textit{pesoMínimo} de $w$.
    \vspace{-0.5\baselineskip}
    \begin{lstlisting}[
        language={[brazilian]pseudocode},
        style=pseudocode,
...
\begin{programruledcaption}{\texttt{procureNóIncideArestaDePesoMínimo($R$, $w$)} \label{prog:findMinWeightEdge}}
    \noindent\textbf{Entrada}: Recebe um nó $w$ de uma floresta com o  \textit{arestasReservasDeNível} $> 0$ e as listas de adjacências $R$.
    \\
...
\begin{programruledcaption}{\texttt{substituaArestaMSF($G$, $i$, $u$, $v$)} \label{prog:replaceMSF}}
    ...
    \textbf{para} j := i \textbf{até} $L$ \textbf{faça}
        ...
        \textbf{enquanto} $T_u$.arestasReservasDeNível > 0     \textbf{faça}
            ...
            $T_u$ := nóXX
            ($x$, $x$) := nóXX.vértices
            ($y$, $w$) := \texttt{extraiaMinLAMSF($R_j$, $x$)} 
...
A linha $13$ é onde acionamos o método \texttt{procureNóIncideArestaDePesoMínimo}, para obtermos o nó de vértice incidente a uma aresta reserva de peso mínimo. Assim, extraímos esta aresta chamando \texttt{extraiaMinLAMSF} na linha $17$, onde obtemos um par ($y$, $w$). 
