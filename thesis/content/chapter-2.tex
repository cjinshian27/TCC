...
Veja que na rotina \texttt{testeSubstitutaMSF} precisamos atualizar a variável \textit{pesoMSF} de $G$ quando encontramos uma aresta substituta, além de chamar os devidos métodos auxiliares (acionamos \texttt{removaLAMSF} e \texttt{adicioneLAMSF} em vez de \texttt{removaLA} e \texttt{adicioneLA}). Além disso, como estamos rebaixando arestas reserva, precisamos atualizar o atributo \textit{pesoMínimo} dos nós de vértice afetados, acionando a rotina \texttt{atualizePesoMínimo} nas linhas $2$, $3$, $8$ e $9$.

Veja que, no Programa~\ref{prog:find-substitute-MSF}, as linhas $4$ a $12$ possuem custo amortizado $\Oh(\lg n)$. Isso quer dizer que, enquanto as arestas que estamos testando não forem substitutas, \texttt{testeSubstituta} será acionado várias vezes com esse custo de tempo. No momento em que encontrarmos uma substituta, o método \texttt{testeSubstituta} será acionado uma única vez e consumirá tempo amortizado $\Oh(\lg^2 n)$ por causa das linhas $16$ e $17$, e assim o algoritmo será finalizado. 