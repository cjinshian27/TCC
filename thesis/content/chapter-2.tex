...
\subsection{Uma observação sobre a busca por uma aresta substituta}

Agora, veremos em detalhes o motivo de não começarmos a procurar primeiro uma aresta substituta nos níveis $> i$ quando removemos uma aresta $uv$ de nível $i$. Será que não há uma aresta substituta no nível $i + 1$ que seja mais leve que uma substituta do nível $i$? 

Primeiramente, é fácil perceber que, se removermos uma aresta de nível $i$ de $F_i$ e $i \neq L$, sabemos que alguma aresta de nível $i + 1$ da floresta $F_{i + 1}$ já tinha sido removida antes, o que causou o rebaixamento de algumas arestas de nível $i + 1$ para o nível $i$. Lembre-se que somente a remoção de arestas rebaixa arestas, visto que a inserção de uma aresta só ocorre em $F_L$ e as florestas de níveis inferiores não são afetadas.
...
Logo na imagem abaixo, temos a aresta $ad$ escolhida para ser uma substituta de menor peso que reconecta $T_u$ e $T_v$, tornando-se uma aresta da floresta. Note que, em $R_i$, as arestas reserva com as duas pontas em $T_u$ de $F_i$ (neste caso, $ub$ e $uc$) possuem peso menor que as arestas reserva com duas pontas em $T_u$ de $F_{i+1}$ (meste caso, $ac$). Isso acontece porque, quando percorremos as arestas reserva em ordem crescente de peso, aproveitamos para rebaixar aquelas com as duas pontas em $T_u$ até encontrarmos uma que conecte $T_u$ e $T_v$.

Ainda na mesma imagem, a aresta \textit{bc} está prestes a ser removida, o que acabaria quebrando uma componente da floresta $F_i$ em $T_b$ e $T_c$. Para reconectar $T_b$ e $T_c$, percorreríamos as seguintes arestas reserva, nesta ordem: $ub$,  $uc$ e $ac$. Veja que, embora $ac$ seja uma substituta que reconecte $T_b$ e $T_c$, a $uc$ seria escolhida em seu lugar por ser a de menor peso, e está em $R_i$. 
...
\begin{figure}[H]
    ...
    \begin{minipage}[c]{0.8\textwidth}
        ...
        \end{tikzpicture}
    \end{minipage}
    \caption{Representação do grafo com a aresta substituta $ad$ por ser a de menor peso em $R_4$ que conecta $T_u$ a $T_v$, tornando-se uma aresta da floresta $F_4$. A próxima aresta a ser removida é \textit{bc}, que está tracejada, acaba de quebrar uma componente de $F_i$ em $T_b$ e $T_c$.}
    \vspace{-1cm}
    \label{fig:why-not-get-replacement-in-higher-levels-2}
\end{figure}

Até o momento, mostramos que arestas reserva de nível $i$ com as duas pontas em $T_u$ possuem peso menor ou igual ao peso das arestas reserva de nível $i+1$ com ambas as pontas em $T_u$.  
No caso das arestas reserva com uma ponta incidente a $T_u$ e outra em $T_v$, nenhuma chega a ser rebaixada, visto que a de menor peso já se tornaria uma aresta substituta e as outras nem seriam percorridas. Por fim, as arestas reserva com ambas as pontas em $T_v$ não são rebaixadas visto que nenhuma delas são percorridas durante a busca por uma substituta.

Com isso, cria-se uma certa intuição de que qualquer aresta reserva incidente a um vértice é mais leve que outra reserva de nível superior incidente ao mesmo vértice. Dessa forma, podemos estabelecer uma quarta invariante para o problema da MSF decremental:

\begin{enumerate}[label=(\Roman*), start = 4]
    \item \label{invariant44} Toda arestas reserva de nível $i$ incidente a um vértice $u$ tem peso menor ou igual ao peso das arestas reserva de nível maior que $i$ incidentes a $u$. 
\end{enumerate}

Esta quarta invariante, apesar de não ter sido utilizada junto com outras três invariantes, evidencia ainda mais a diferença  entre o algoritmo de conexidade dinâmica descrito no Capítulo~\ref{chapter:connectivity-chapter} e o algoritmo para MSF decremental, além de mostrar como as arestas reserva se comportam ao longo dos $L$ níveis do grafo $R$ de acordo com o peso delas.

\subsection{Consumo de tempo de \texttt{substituaArestaMSF}}
