...
\end{programruledcaption}
 ...
 Embora este método não esteja disponível para o usuário, é importante entender os ajustes realizados, visto que eles serão necessários para o funcionamento correto do método \texttt{substituaArestaMSF}. Veja que a complexidade de tempo da versão final de \texttt{adicioneMSF} continua sendo  $\Oh(\lg n)$, e as três invariantes também permanecem preservadas.
...
Para descrever a rotina \texttt{substituaArestaMSF}, descrita no Programa~\ref{prog:replaceMSF}, usaremos vários dos métodos auxiliares já apresentados, além dos campos novos adicionados aos nós das florestas, mostrados na Seção~\ref{sec:forest-node-adjustments}. Mostraremos também a rotina \texttt{testeSubstitutaMSF}, que é uma versão ajustada da rotina \texttt{substituaAresta} que vimos antes. 
