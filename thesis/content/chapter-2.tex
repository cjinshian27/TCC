...
Primeiramente, é fácil perceber que, se removermos uma aresta de nível $i$ de $F_i$ e $i \neq L$, sabemos que alguma aresta de nível $i + 1$ da floresta $F_{i + 1}$ já tinha sido removida antes, o que causou o rebaixamento de algumas arestas de nível $i + 1$ para o nível $i$. Lembre-se não há inserção depois que as remoções (e rebaixamentos) começam a ocorrer.
...
Ainda na Figura~\ref{fig:why-not-get-replacement-in-higher-levels-2}, a aresta \textit{bc}, de nível $i$, está prestes a ser removida, o que acabaria quebrando uma componente da floresta $F_i$ em $T_b$ e $T_c$. Para reconectar $T_b$ e $T_c$, observe que apenas $uc$ seria percorrida e já é substituta. Embora $ac$ e $cv$ também sejam substitutas incidentes a $T_c$, pelo fato de elas estarem em $R_{i+1}$ e pela invariante~\ref{invariant44}, são mais pesadas que $uc$, que está em $R_i$. 
...
\begin{figure}[H]
    ...
    \begin{minipage}[c]{0.8\textwidth}
        ...
        \end{tikzpicture}
    \end{minipage}
    \caption{Representação do grafo com a aresta substituta $ad$ por ser a de menor peso em $R_4$ que conecta $T_u$ a $T_v$, tornando-se uma aresta da floresta $F_4$. A próxima aresta a ser removida é \textit{bc}, que está tracejada, e quebra uma componente de $F_i$ em $T_b$ e $T_c$.}
    \vspace{-1cm}
