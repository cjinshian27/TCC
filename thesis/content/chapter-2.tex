...
Na Figura~\ref{fig:example-replacement-msf3}, percorremos as arestas reserva em ordem de peso em $R_4$ que tenham uma das pontas em~$T_u$. Assim, olhamos para as arestas reserva, que não são substitutas, em $R_4$ nesta ordem: $df$ (peso $10$), $dh$ (peso $11$), $bu$ (peso $12$), $uc$ (peso $13$). Para cada aresta percorrida, verificamos se a outra ponta dela incide em algum vértice de $T_v$. Caso não incida, a aresta tem duas pontas em $T_u$, pois $F_4$ era maximal antes da remoção de $uv$, e logo a aresta não é uma substituta.  Então a rebaixamos para o nível $3$, ou seja, movemos de $R_4$ para $R_3$ as arestas  percorridas que não são substitutas.
...
O método \texttt{removaMSF} está descrito abaixo. Como se pode ver, não há alterações em sua estrutura se comparado com \texttt{removaGD}, exceto na mudança de nomes de alguns métodos.
...
O método \texttt{substituaArestaMSF} será descrito mais em frente, mas veremos que ele consome tempo amortizado $\Oh(\lg^2 n)$, e assim \texttt{removaMSF} também irá consumir tempo amortizado $\Oh(\lg^2 n)$.
