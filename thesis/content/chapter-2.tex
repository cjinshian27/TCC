...
Agora, veremos em detalhes o motivo de não precisarmos procurar uma substituta de menor peso em níveis $> i$ quando removemos uma aresta $uv$ de nível $i$. 
...
\begin{figure}[H]
    ...
    \begin{minipage}[c]{0.8\textwidth}
        ...
        reserve/.style={red, thick},
        removed/.style={black, thick, dashed},
        reserveremoved/.style={red, thick, dotted}]
    ...
    \begin{minipage}[c]{0.8\textwidth}
        ...
        \end{tikzpicture}
    \end{minipage}
    \caption{Representação da busca por uma aresta substituta em $R_{i+1}$. As arestas reserva de nível $i+1$ que estão pontilhadas foram percorridas em ordem crescente de peso e estão prestes a serem removidas de $R_{i+1}$, pois foram rebaixadas para o nível $i$, como se pode ver em $R_{i}$.}
    \vspace{-1cm}
    \label{fig:why-not-get-replacement-in-higher-levels-1}
\end{figure}

Logo na imagem abaixo, temos a aresta $ad$ escolhida como uma substituta de menor peso que reconecta $T_u$ e $T_v$. Note que, em $R_i$, as arestas reserva com as duas pontas em $T_u$ de $F_i$ (neste caso, $ub$ e $uc$) possuem peso menor que as arestas reserva com duas pontas em $T_u$ de $F_{i+1}$. Isso acontece porque rebaixamos as aresta reserva com as duas pontas em $T_u$ em ordem crescente de peso até encontrarmos uma que conecte $T_u$ e $T_v$.
