...
A versão completa do método \texttt{adicioneMSF} está descrita abaixo. Embora este método não esteja disponível para o usuário, é importante entender os ajustes realizados, visto que eles serão necessários para o funcionamento correto do método \texttt{substituaArestaMSF}. 
...
Para descrever a rotina \texttt{substituaArestaMSF}, descrita no Programa~\ref{prog:replaceMSF}, usaremos vários dos métodos auxiliares já apresentados, além de alguns métodos e campos novos adicionados aos nós das florestas, como vimos na Seção~\ref{sec:forest-node-adjustments}. Mostraremos também a rotina \texttt{testeSubstitutaMSF}, que é uma versão ajustada da rotina \texttt{testeSubstituta} que vimos antes. 
