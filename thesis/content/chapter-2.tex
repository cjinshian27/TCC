...
Para o algoritmo da MSF decremental, além destes campos apresentados, precisaremos de dois campos extras para cada nó da floresta: \textit{peso} e \textit{pesoMínimo}. O primeiro campo indica o peso de um nó de aresta (nós de vértice guardam neste campo o valor $\infty$). O segundo campo guarda o peso mínimo dentre todas as arestas reserva em cada $R_i$, pois é assim que conseguimos extrair a aresta reserva de peso mínimo quando estamos buscando uma substituta. 

Para entendermos como estes dois campos extras aparecem em cada nó da árvore, usaremos um exemplo de um grafo ponderado $G$ de $5$ vértices e $7$ arestas ponderadas, como se pode ver na Figura~\ref{fig:generic-graph-example-1}. 
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \node[node, label=below:{1}] (dd) at (1.5, -4.5) {$dd$};
        \node[node, label=below:{1}] (cc) at (4.5, -4.5) {$cc$};
        ...
        \node[node, label=below:{1}] (ee) at (-7.5, -4.5) {$ee$};
        \node[node, label=below:{1}] (aa) at (-4.5, -4.5) {$aa$};


        \node[node, label=below:{1}] (bb) at (-3, -3) {$bb$};
    ...
    \end{tikzpicture}
    \caption{Árvore de uma das componentes da floresta $F_i$, onde arestas pretas são da floresta e as vermelhas são arestas reserva.}
