...
Agora, veremos em detalhes o motivo de não precisarmos procurar uma substituta de menor peso em níveis menores que $i$ quando removemos uma aresta $uv$ de nível $i$. Veja que, como a aresta $uv$ tem nível $i$, ela não pertence a $F_{i-1}$. Logo, pela invariante~\ref{invariant22}, $u$ e $v$ estão em componentes distintas de $F_{i-1}$. Como $F_{i-1}$ é uma MSF de $G_{i-1}$, não existe aresta reserva de nível $\leq i - 1$ de menor peso que conecte as componentes $T_u$ e $T_v$. Portanto, só procuramos uma substituta de menor peso em níveis $\geq i$. A Figura~\ref{fig:why-not-search-in-less-or-equal-than-i-in-msf} torna a explicação mais intuitiva.
...
\begin{figure}[H]
    ...
    \node[text=red] at (-0.5, 1.2) {$3$};
    \node[text=green] at (0, -0.4) {$7$};
    ...
    \end{tikzpicture}
    \caption{Os círculos verdes representam as componentes $T_u$ e $T_v$ da floresta $F_i$ do grafo $G_i$, enquanto os círculos vermelhos representam as componentes da floresta $F_{i-1}$ do grafo $G_{i-1}$. Note que a aresta reserva de nível $i-1$ de peso $3$ mostrada não deveria existir, porque senão ela violaria a invariante~\ref{invariant11}, já que poderíamos simplesmente substituir a aresta verde pela vermelha e obter uma MSF de menor peso para $G_i$, mas a aresta vermelha é de nível $i-1$. Portanto, tais arestas de nível $i-1$ ligando componentes de $F_i$ (como a aresta vermelha de peso $3$) não existem e só é necessário procurar arestas substitutas a partir de nível $i$ (como a aresta verde de peso $7$) para conectar $T_u$ e $T_v$.}
    \label{fig:why-not-search-in-less-or-equal-than-i-in-msf}
\end{figure}
...
Isso quer dizer que a inserção da aresta $e$ vai pagar por cada execução das linhas $7$ a $10$ do Programa~\ref{prog:replaceMSF} e das linhas $4$ a $12$ do Programa~\ref{prog:find-substitute-MSF} que processa a aresta $e$. Como a inserção custa $\Oh(\lg n)$ e essas linhas custam $\Oh(\lg n)$ e são executadas $\Oh(\lg n)$ vezes, pois $e$ pode ser rebaixada no máximo $\left\lceil \lg n \right\rceil$ vezes, o custo pago por uma inserção é $\Oh(\lg^2 n)$.

Já uma remoção de aresta, executada pelo Programa~\ref{prog:removeGD-version-msf-final-version}, custa $\Oh(\lg^2 n)$ mais o custo do \texttt{substituaArestaMSF}. O custo do \texttt{substituaArestaMSF} é $\Oh(\lg^2 n)$ excluindo-se as execuções das linhas $7$ a $10$ do Programa~\ref{prog:replaceMSF}, assim como as linhas $4$ a $12$ do Programa~\ref{prog:find-substitute-MSF}. Isso porque, desconsiderando estas linhas onde ocorrem rebaixamentos de arestas, para cada execução da linha $2$ do Programa~\ref{prog:replaceMSF} acaba tendo custo $\Oh(\lg n)$ nas linhas restantes. Apesar das linhas $16$ e $17$ do Programa~\ref{prog:find-substitute-MSF} consumirem tempo $\Oh(\lg^2 n)$, este trecho do código será executado uma única vez quando encontrarmos uma substituta e o algoritmo finaliza. Assim, como a linha $2$ pode ser executada no máximo $\Oh(\lg n)$ vezes, temos que \texttt{substituaAresta} consome tempo amortizado $\Oh(\lg^2 n)$ por operação de remoção.
