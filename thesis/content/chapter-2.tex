...
\begin{itemize}
    ...
    \item \texttt{\textbf{extraiaMinLAMSF(R, u)}}: remove e retorna o par $\{v, p\}$, onde $v$ é vértice do min-heap de $u$ em $R$ com chave mínima $p$.
\end{itemize} 

Uma chamada à rotina \texttt{adicioneLAMSF(R, u, v, p)} adiciona o par $\{u, p\}$ no min-heap de $v$, como também adiciona o par $\{v, p\}$ no min-heap de $u$, consumindo tempo $\Oh(\lg n)$. Similarmente, uma chamada à rotina \texttt{removaLAMSF(R, u, v)} remove o par $\{u, p\}$ do min-heap de $v$, como também remove o par $\{v, p\}$ do min-heap de $u$, consumindo também tempo $\Oh(\lg n)$. 
