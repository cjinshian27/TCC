...
Já uma remoção de aresta, executada pelo Programa~\ref{prog:removeGD-version-msf-final-version}, custa $\Oh(\lg^2 n)$ mais o custo do \texttt{substituaArestaMSF}. O custo do \texttt{substituaArestaMSF} é $\Oh(\lg^2 n)$ excluindo-se as execuções das linhas $7$ a $10$ do Programa~\ref{prog:replaceMSF}, assim como as linhas $7$ a $14$ do Programa~\ref{prog:find-substitute-MSF}. Isso porque, desconsiderando estas linhas onde ocorrem rebaixamentos de arestas, para cada execução da linha $2$ do Programa~\ref{prog:replaceMSF} acaba tendo custo $\Oh(\lg n)$ nas linhas restantes. Apesar das linhas $18$ e $19$ do Programa~\ref{prog:find-substitute-MSF} consumirem tempo $\Oh(\lg^2 n)$, este trecho do código será executado uma única vez quando encontrarmos uma substituta e o algoritmo finaliza. Assim, como a linha $2$ pode ser executada no máximo $\Oh(\lg n)$ vezes, temos que \texttt{substituaAresta} consome tempo amortizado $\Oh(\lg^2 n)$ por operação de remoção.
