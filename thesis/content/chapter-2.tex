...
A seguir, temos o método \texttt{procureNóIncideArestaDePesoMínimo}, descrito no Programa~\ref{prog:findMinWeightEdge}. Este método, como o nome sugere, procura e retorna o nó de vértice que incide em uma aresta reserva de peso mínimo. Ele será utilizado no método \texttt{substituaArestaMSF}, que descreveremos na Seção~\ref{sec:replaceMSF}.
...
Quando inserimos uma aresta reserva $uv$ em $R_L$, precisamos atualizar o atributo \textit{pesoMínimo} dos nós de vértice $u$ e $v$ de $F_L$, como se pode ver nas linhas $6$ e $7$ do Programa~\ref{prog:addMSF-final-version}. Isso porque se o peso de $uv$ é o menor dentre todas as arestas reserva inseridas em $R_i$ até o momento, então o atributo \textit{pesoMínimo} de $u$ e de $v$ passa a ser o peso de $uv$. 
...
Veja que a complexidade de tempo da versão final de \texttt{adicioneMSF} continua sendo $\Oh(\lg n)$, e as três invariantes também permanecem preservadas.

\subsection{Versão completa da rotina de remoção de arestas}
\label{sec:removeMSF-finalVersion}

A versão completa do método \texttt{removaMSF} está descrita abaixo. Quando removemos uma aresta reserva $uv$ de $R_i$, precisamos atualizar o atributo \textit{pesoMínimo} dos nós de vértice $u$ e $v$ de $F_i$, como se pode ver nas linhas $12$ e $13$ do Programa~\ref{prog:removeGD-version-msf-final-version}. Isso porque se o peso de $uv$ era o menor dentre todas as arestas reserva restantes, então o atributo \texttt{pesoMínimo} de $u$ e de $v$ passa a ser o peso da aresta reserva de segundo menor peso em $R_i$. 
