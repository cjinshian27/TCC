...
Inicialmente, avaliamos o grafo $G_0$ considerando a remoção de $25\,600$ arestas em cinco permutações distintas. Para cada permutação, registramos o peso da árvore geradora mínima (MST) após a remoção de cada lote de arestas e o tempo de execução, em milissegundos, dos algoritmos de Kruskal e MSF Decremental. Depois, calculamos a média dos cinco testes realizados. Por fim, realizamos os mesmos experimentos de $G_0$  em outros cinco grafos, usando somente a primeira permutação de arestas gerada para cada grafo. Os resultados foram exibidos em tabela e gráfico feitos na linguagem \textit{Python3}.
...
A partir dos experimentos realizados, verifica-se que os grafos gerados para os testes apresentam um número de arestas próximo ao valor esperado para um grafo do modelo $G(n, p)$. Ademais, os resultados indicam que, nos gráficos de tempo de execução, o algoritmo de Kruskal apresenta um crescimento aproximadamente linear em função do número de arestas removidas, enquanto o algoritmo de MSF decremental exibe um padrão de crescimento logarítmico.

As tabelas de resultados complementam essa análise ao evidenciar de forma quantitativa a diferença significativa no tempo de execução entre os dois algoritmos durante o processo de remoção de arestas, diferença esta que já se mostra perceptível mesmo nas primeiras remoções realizadas.
